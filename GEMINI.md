...
- **Key Modules:** 
  - `core_k0/agent_loop_k2.py`: K2 agent with "Experience -> Strategy" logic.
  - `core_k0/metrics.py`: Paper-compliant formulas for Dissonance and Identity.
  - `experiments_k0/`: Multi-phase environments (Resource Survival, Classroom).
...
```powershell
python core_k0/long_horizon_stability_test_v2.py --episodes 100000 --checkpoint-interval 10000
```

### **2. Classroom Data Pilot (Fairness)**
Tests bias mitigation on synthetic student interaction patterns.
```powershell
python generate_student_data.py  # Step 1: Generate Data
python classroom_data_pilot.py   # Step 2: Run Pilot
```

### **3. Adversarial Stress-Testing**
Tests robustness against reward hacking and malformed inputs.
```powershell
python adversarial_safety_test.py
...
```powershell
python consolidate_research.py
```
...
## 📁 Key Files
- `RAVANA_v2_Manuscript.tex`: The formal LaTeX source for the research paper.
- `RAVANA_v2_FINAL_PACKAGE.md`: Consolidated archival package of all findings.
- `results/`: Contains verified empirical data and checkpoints.
- `core_k0/agent_loop_k2.py`: The "Brain" of the system.
